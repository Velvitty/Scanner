...
디시인사이드 유저는 사용을 금하며, 이는 라이선스로, 법적 효력을 갖습니다.
 위반 시, 디시인사이드 내 IP와 비교하여 법적 형사 고소를 가할 것 입니다.
...
5. **내보내기**로 지금 사진을, **전체 ZIP**으로 모든 사진을 한꺼번에 저장합니다. 전체 ZIP은 도중에 **취소**하면 그때까지 변환한 사진만 담아 저장합니다. Ai+ 켜짐, 저장 형식, JPEG 품질은 다음에 열 때도 그대로입니다.
...
$`\lvert f\rvert \le 0.12`$ 인 보통 장면은 모두 같은 밝기(로그 평균이 선형 0.26, L* 약 58)로 맞추고, 눈밭이나 야경처럼 분명한 장면만 그 키를 따릅니다. 로우키 쪽의 2차 항은 조금 어두운 장면(실내등, $`f_d \approx -0.08`$)은 거의 그대로 두고 야경처럼 아주 어두운 장면($`f_d \approx -0.56`$)만 더 어둡게 남깁니다. 합성 야경의 평균 L*가 32에서 25로 내려갔습니다(정답 15, 기존 방식 14). 폰 카메라처럼 조금 밝히는 쪽에 남겨 두었습니다.
...
```math
\mathcal L(v_{lo}) = \mathrm{logit}(b_T),\qquad \mathcal L(v_{hi}) = \mathrm{logit}(0.95),\qquad n_s \in [1.45,\ 1.7],\quad n_h \in [1.5,\ 2.0]
```

```math
v_{lo} = (l_{lo} - \bar\ell)\ln 2,\quad v_{hi} = (l_{hi} - \bar\ell)\ln 2,\qquad
b_T = 0.004 \cdot 2^{\,0.7\,\mathrm{clip}(8 - DR,\ 0,\ 5)}
```
...
```math
\sigma = \mathrm{clip}\left(\sqrt{30 / C^*_{75}},\ 1.0,\ 1.3\right)
```
...
| Ai+ 화밸 | `aiCast`, `aiCastGather`, `aiCastBands`, `aiCastFit`, `aiCastTable`, `smooth2` | 중성 후보, 구간 봉우리, 합의 적합, 보정 표 |
...
| 지표 | 기존 방식 | Ai+ |
|---|---|---|
| 색상 오차 (유채색 패치 평균) | 5.7° | **3.6°** |
| 기준 대비 채도 | 1.11 ± 0.09 | **1.16 ± 0.07** |
| 평균 밝기 L* (장면 간 편차) | 63.1 ± 9.7 | **58.8 ± 5.3** |
| 중간 회색 패치 L* | 54.4 ± 10.9 | **47.2 ± 5.9** |
| 대비 (L* 표준편차) | 18.3 | **22.7** |
| 평탄한 패치의 잡음 (8비트 표준편차) | 1.40 | **0.77** |
| 회색 패치 C* 평균 (최대) | 3.9 (11.8) | **4.3 (9.9)** |
...
|---|---|---|---|
| 야경 | 7.1 | 12.9 | **2.5** |
| 텅스텐 / B형 텅스텐 | 11.4 / 8.7 | 12.1 / 9.7 | **6.5 / 6.8** |
| 노을 / 숲 | 3.7 / 2.7 | 1.7 / 4.0 | **1.2 / 1.8** |
| 주광 / 2스톱 과다 | 1.3 / 1.2 | 1.9 / 2.0 | **1.0 / 1.0** |
| C형 주광 (크로스오버, 기존 a* −9) | 7.6 | — | **4.6** |
| C형 그늘 / 인물 | 6.8 / 5.4 | — | **5.0 / 3.0** |
| C형 숲 / 텅스텐 | 10.4 / 18.3 | — | 11.4 / 18.1 |
| 형광등 | 3.2 | 3.7 | 9.0 |
| 그늘 / 인물 | 4.3 / 0.9 | 5.8 / 2.0 | 6.4 / 2.3 |
| 원래 15종 평균 | 3.3 | 4.2 | **3.1** |
| C형 5종 평균 | 9.7 | — | **8.4** |
...
22종 전체로는 평균 L*가 62.8 ± 17.1에서 58.5 ± 9.7로, 흰색 클리핑이 5.0%에서 7.7%로 바뀝니다. Ai+의 흰색 클리핑은 대부분 밝은 파란 하늘의 파랑 채널, 노을의 태양 주변, 역광의 하늘처럼 디지털 사진에서도 포화되는 곳입니다.
...
| 작업 | 기존 방식 | Ai+ |
|---|---|---|
| 불러오기 + 분석 + 첫 렌더 | 1.22초 | 2.3초 |
| 슬라이더 한 번 (미리보기 렌더) | 약 60ms | 약 210~420ms |
| Ai+ 분석 (지도 새로 / 재사용) | — | 479ms / 376ms |
| JPEG 저장 | 3.3초 | 7.2초 |
| PNG16 저장 | 11.0초 | 13.5초 |
| JS 힙 | — | 254MB |
